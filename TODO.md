...
## Next Actions
- [ ] `from_ast` の本実装（full_moonトークン/トリビアから `---@` コメントを収集し直後ステートメント行に割当）
  - [ ] 空行や通常コメントを跨いだ連続ブロックの扱い（現行互換）
  - [ ] 先頭や孤立の `---@class`/`---@enum`/`---@field` の登録（TypeRegistry）
  - [ ] パフォーマンス検討（1ファイル内の一回走査・キャッシュ）
- [ ] `from_ast` 専用の単体テストを追加
  - [ ] 直前ブロックと割当の境界（空行あり/なし、複数行）
  - [ ] ステートメントなしのヘッダファイル（クラス定義のみ）
  - [ ] 末尾コメントが次行コードに紐付かないことの確認
