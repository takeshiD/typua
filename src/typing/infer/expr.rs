...
use crate::typing::types::{
    Params, Prim, Scheme, Subst, TyVarId, Type, apply, generalize, instantiate,
};
...
mod tests {
    ...
    fn infer_identity_call() {
        // id : ∀T. fun(T): T
        let t_fun = Type::Fun {
            params: Params::Fixed(vec![v(0)]),
            returns: Params::Fixed(vec![v(0)]),
        };
        ...
        assert_eq!(
            format!("{}", crate::typing::types::TypeDisplay::new(&ret)),
            "number"
        );
    }
    ...
    fn infer_map_call() {
        ...
        let f = Type::Fun {
            params: Params::Fixed(vec![t.clone()]),
            returns: Params::Fixed(vec![u.clone()]),
        };
        let arr_t = Type::Table(crate::typing::types::Table::Map {
            key: Box::new(prim(Prim::Integer)),
            value: Box::new(t.clone()),
        });
        let arr_u = Type::Table(crate::typing::types::Table::Map {
            key: Box::new(prim(Prim::Integer)),
            value: Box::new(u.clone()),
        });
        let map_ty = Type::Fun {
            params: Params::Fixed(vec![f, arr_t.clone()]),
            returns: Params::Fixed(vec![arr_u.clone()]),
        };
        ...
        let fn_arg = Type::Fun {
            params: Params::Fixed(vec![prim(Prim::Number)]),
            returns: Params::Fixed(vec![prim(Prim::String)]),
        };
        let arr_num = Type::Table(crate::typing::types::Table::Map {
            key: Box::new(prim(Prim::Integer)),
            value: Box::new(prim(Prim::Number)),
        });
